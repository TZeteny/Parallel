...
int main() {
    ...
    //Search without threading
    t = clock(); //start measuring
    search_all(filename, search);
    t = clock() - t; //stop clock
    runtime1 = ((double)t)/CLOCKS_PER_SEC;
    ...
    t = clock() - t; //stop clock
    runtime2 = ((double)t)/CLOCKS_PER_SEC;
    printf("Unthreaded runtime: %lfs\n", runtime1);
    printf("Threaded runtime: %lfs\n", runtime2);

	return 0;
}

void search_all(char* filename, char* search) {
